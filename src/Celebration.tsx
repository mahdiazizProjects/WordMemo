...
export function Celebration({ title, subtitle, xp, enabled = true, kind = 'correct' }: {
  ...
  useEffect(() => {
    ...
    const animation = Animated.timing(progress, { toValue: 1, duration: 850, easing: Easing.out(Easing.cubic), useNativeDriver: Platform.OS !== 'web' });
    animation.start();
  ...
  return <View style={styles.card} accessibilityLiveRegion="polite">
    <View style={styles.art} accessible={false}>
      {animate && kind !== 'practice' && Array.from({ length: kind === 'complete' ? 16 : 10 }, (_, index) => {
        const angle = index * Math.PI * 2 / (kind === 'complete' ? 16 : 10);
        return <Animated.View key={index} style={[styles.spark, { backgroundColor: index % 2 ? C.gold : C.green,
          opacity: progress.interpolate({ inputRange: [0, .55, 1], outputRange: [0, 1, 0] }),
          transform: [{ translateX: progress.interpolate({ inputRange: [0, 1], outputRange: [0, Math.cos(angle) * 85] }) },
            { translateY: progress.interpolate({ inputRange: [0, 1], outputRange: [0, Math.sin(angle) * 62] }) }, { rotate: `${index * 37}deg` }] }]} />;
      })}
...
const styles = StyleSheet.create({
  ...
  art: { width: 210, height: 108, justifyContent: 'center', alignItems: 'center' },
  medal: { width: 74, height: 74, borderRadius: 37, backgroundColor: C.pale, borderWidth: 1, borderColor: '#CFDAC7', alignItems: 'center', justifyContent: 'center' },
  spark: { position: 'absolute', width: 6, height: 10, borderRadius: 2 },
