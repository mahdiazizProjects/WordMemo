import { GardenPlant } from './Garden';
...
type Member = { userId: string; displayName: string; role: string; shareProgress: boolean; shareQuest?: boolean; summary?: { verses: number; practiceDays: number; xp: number; level: number } };
type SharedStack = { id: string; name: string; verseIds: string[]; authorId: string; createdAt: string };
type Group = { quest?: { week: string; target: number; count: number; participants: number }; id: string; name: string; ownerId: string; me: Member; members: Member[]; stacks: SharedStack[]; roomId?: string };
...
export function GroupsScreen({ store, mode, editor, setEditor, onGroups, onCards, onBrowse, onPageChange, onLearn }: { store: StoredState; mode: 'groups' | 'stacks'; editor?: VerseStack; setEditor: Dispatch<SetStateAction<VerseStack | undefined>>; onGroups: () => void; onCards: () => void; onLearn: (stack: VerseStack) => void; onBrowse: (scope: 'all' | 'learning' | 'saved') => void; onPageChange: () => void }) {
  ...
  if (groupId) return <>
    ...
    {!group ? <><Text style={U.body}>Opening your group…</Text><Button variant="secondary" disabled={busy} onPress={() => { void run(() => loadGroup()); }}>Try again</Button></> : <>
      ...
      {group.quest && <View style={U.card}><Text style={U.eyebrow}>This week · Grow together</Text><Text style={U.sectionTitle}>Tend the Word together.</Text><GardenPlant grown={group.quest.count > 0} bloom={group.quest.count >= group.quest.target} enabled={state.settings.celebrationsEnabled !== false} /><Text style={U.body}>Complete {group.quest.target} daily reviews together this week. Up to five reviews per person each day count, including attempts you’re still learning.</Text><GrowBar value={Math.min(100, group.quest.count / group.quest.target * 100)} label="Group quest progress" enabled={state.settings.celebrationsEnabled !== false} /><Text accessibilityLiveRegion="polite" style={U.body}>{group.quest.count} / {group.quest.target} Reviews · {group.quest.participants} Participants</Text>{group.quest.count >= group.quest.target && <Celebration title="Your group is in bloom!" subtitle="You reached this week’s shared goal together." xp={0} kind="complete" enabled={state.settings.celebrationsEnabled !== false} />}<Text style={U.small}>Week beginning {group.quest.week}. A new quest begins each Monday (UTC). Counts appear after progress syncs. Leaving or opting out removes your contribution from the current total.</Text><View style={U.between}><Text style={[U.body, { flex: 1 }]}>Contribute to this group quest</Text><Switch accessibilityLabel="Contribute to this group quest" value={!!group.me.shareQuest} disabled={busy} onValueChange={value => { void run(async () => { await learningStore.sync(); await api(`/groups/${groupId}/membership`, { displayName: group.me.displayName, shareProgress: group.me.shareProgress, shareQuest: value }, 'PUT'); await loadGroup(); }); }} trackColor={{ true: C.green, false: C.line }} /></View><Text style={U.small}>Optional. Your weekly count contributes to the group total. Your answers and verse choices stay private. In a small group, others may be able to infer your contribution.</Text><Button variant="secondary" onPress={() => { void run(async () => { await learningStore.sync(); await loadGroup(); }); }} disabled={busy}>Sync and refresh quest</Button></View>}
