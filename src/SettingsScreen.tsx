...
export function SettingsScreen({ state, setState, notify, webAppStatus, store }: { store: StoredState; state: AppState; setState: React.Dispatch<React.SetStateAction<AppState>>; notify: (text: string) => void; webAppStatus: WebAppStatus }) {
  ...
  return <>
    ...
    {Platform.OS === 'web' && <View style={U.card}><Text style={U.sectionTitle}>Keep the Word close</Text><Text style={U.body}>Add WordMemo to your home screen for an app you can open with one tap.</Text><Text style={U.small}>iPhone or iPad: open in Safari, tap Share, then Add to Home Screen. Android: open the browser menu and choose Install app or Add to Home screen.</Text><Text accessibilityLiveRegion="polite" style={U.small}>{webAppStatus.offline === 'ready' ? 'Offline Bible ready. Reading, search, and practice are available without a connection while this browser keeps the downloaded files.' : webAppStatus.offline === 'preparing' ? 'Preparing the full Bible for offline use. Keep this page open with an internet connection until it is ready.' : 'Offline loading is unavailable right now. Keep an internet connection to reopen the app.'}</Text>{webAppStatus.updateAvailable && <Text style={U.small}>An update is ready. After finishing practice, close every WordMemo tab and app window, then reopen it to use the update.</Text>}</View>}
